export const states = { // we use enum to define a set of text constants
    STANDING_LEFT: 0,
    STANDING_RIGHT: 1,
    SITTING_LEFT: 2,
    SITTING_RIGHT: 3,
...
export class StandingLeft extends State {
    ...
    enter(){ // runs once when status is entered
    ...
    handleInput(input){
        if (input === 'PRESS left') // set state to StandingRight
...
export class JumpingLeft extends State {
    ...
    enter(){ // runs once when status is entered
        ...
        //no enter horizontal speed, we inherit the previous speed when entering jumping state
        this.vy -= 20;
    ...
    handleInput(input){
        if (input === 'PRESS right')
            ...
            this.player.setState(states.STANDING_LEFT);
        else if (this.player.vy > 0) // when velocity turns to positive number it is falling
