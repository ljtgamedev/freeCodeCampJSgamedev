window.addEventListener('load', function() {
    ...
    class Player {
        ...
        draw(context){
            ...
            context.beginPath();
            // hitbox (circle): X coordinate, Y coordinate, radius, ...
        ...
        update(input, deltaTime, enemies){
            ...
            enemies.forEach(enemy => {
                const dx = (enemy.x + enemy.width/2 - 20) - (this.x + this.width/2); // enemy X coordinate - player X coordinate
                const dy = (enemy.y + enemy.height/2) - (this.y + this.height/2 + 20); // enemy Y coordinate - player Y coordinate
                const distance = Math.sqrt(dx*dx + dy*dy);
                if (distance < enemy.width/3 + this.width/3){ // if distance < enemy radius + player radius
                    gameOver = true;
                }
            })
            // sprite animation
            ...
            if ((input.keys.indexOf('ArrowUp') > -1 || input.keys.indexOf('swipe up') > -1) && this.onGround()) { // only jump when on ground
                this.vy -= 32;
            } 
    ...
    function restartGame(){
        ...
        score = 0;
        gameOver = false;
        animate(0); // After game over the game stops animating, when the game is restarted we need to restart te animation loop 
    }
